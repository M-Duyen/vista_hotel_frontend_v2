...
const mappingBookings = async (res: any) => {
    const cancellationId =
        res.cancellationID || res.cancellationId || res.cancellation_id;
    ...
    return {
        ...
        bookingDetails,
        earlyCheckin:
            earlyCheckinRes?.data?.data || earlyCheckinRes?.data || null,
        lateCheckout:
            lateCheckoutRes?.data?.data || lateCheckoutRes?.data || null,
...
export const overlapBookingExists = async (
    ...
    try {
        const res = await bookingsApi.get(
            `/overlapping-bookings/${roomNumber}`,
        );
        return res.data;
    } catch (error) {
...
export const cancelBooking = async (
    ...
    try {
        ...
        if (refundMethod) {
            ...
            // Also provide flat fields for refund if it's an object
            if (typeof refundMethod === 'object') {
                Object.assign(payload, refundMethod);
            }
        }
...
export const getBookingServicesByBookingId = async (bookingId: string) => {
    try {
        const response = await api.get(
            `/booking-services/booking/${bookingId}`,
        );
        return response.data;
    } catch (error) {
