...
const mappingBookings = async (res: any) => {
    ...
    const [
        ...
        res.earlyCheckinID && bookingId
            ? requestApi
                  .get(`/early-checkins/booking/${bookingId}`)
                  .catch(() => null)
            : Promise.resolve(null),
        res.lateCheckoutID && bookingId
            ? requestApi
                  .get(`/late-checkouts/booking/${bookingId}`)
                  .catch(() => null)
            : Promise.resolve(null),
        res.cancellationID
            ? bookingsApi
...
export const getBookingsByCheckInDateRange = async (
    ...
    try {
        ...
        return allBookings.filter((b) => {
            ...
            return (
                checkInTime >= start.getTime() && checkInTime <= end.getTime()
            );
        });
    } catch (error) {
...
export const getBookingsByCheckOutDateRange = async (
    ...
    try {
        ...
        return allBookings.filter((b) => {
            ...
            return (
                checkOutTime >= start.getTime() && checkOutTime <= end.getTime()
            );
        });
    } catch (error) {
